...
model:add(SpatialConvolution(3, 32, 3, 3 ,1,1,1,1,opt.stcWeights ))
model:add(SpatialMaxPooling(2, 2))
model:add(SpatialBatchNormalization(32, opt.runningVal))
model:add(nn.HardTanh())

--model:add(BinarizedNeurons(opt.stcNeurons))

model:add(SpatialConvolution(32, 64, 3, 3,1,1,1,1,opt.stcWeights ))
model:add(SpatialMaxPooling(2, 2))
...
model:add(BinaryLinear(128*4*4,numHid,opt.stcWeights))
...
--[[
model:add(BinaryLinear(128*8*8,numHid,opt.stcWeights))
model:add(BatchNormalization(numHid, opt.runningVal))
model:add(nn.HardTanh())
model:add(BinarizedNeurons(opt.stcNeurons))
--]]

model:add(BinaryLinear(numHid,outputN,opt.stcWeights))
model:add(nn.BatchNormalization(outputN))
